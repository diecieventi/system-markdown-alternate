...
* **Fixed: the first request after a plugin update still honoured date-only revalidation.** `0.51.0` made an update invalidate `If-Modified-Since` the way a settings save does, but `AdminSettings::maybe_bump_for_plugin_version()` only *marks* the salt bump, which `flush_cache_salt()` writes at `shutdown`. `MarkdownController::date_is_strong_validator()` reads the *stored* salt, so the request that detected the update — and any request running before it finished — still saw the pre-update salt, trusted `post_modified_gmt`, advertised `Last-Modified` and answered a pre-update `If-Modified-Since` with a bodyless `304`. Found by the `0.53.0` staging acceptance run and reproduced over real HTTP on both staging sites (upgrade `0.51.0` → `0.53.0`): first request `304`, second `200`. The validator now also refuses the date while the stored `sysmda_version` differs from `SYSMDA_VERSION`, an absent option included — which covers a fresh install and an upgrade from a release that predates the option. The salt write stays deferred to `shutdown` on purpose (it is what keeps a settings save from caching half-old output under the new salt). Cost: one autoloaded option read `AdminSettings` already performs on every request.
* Tests: the pending-upgrade case (a stored older version, and no stored version at all) is asserted on both `Last-Modified` and `If-Modified-Since`, and was seen to fail before the fix; the date assertions around it now state the recorded-version steady state explicitly instead of depending on the option being absent.
* Docs: `docs/output-format.md` states that an upgrade counts from the first request; `docs/staging-acceptance.md`, the acceptance list and the durable decision in `AGENTS.md` now require the upgrade check to be made on the first front-end request after the update, which is the only one that could have caught this.
...
* **Fixed: a password-protected synced pattern was expanded into the Markdown of every post that used it.** `BlockCleaner::expand_reusable()` checked the referenced `wp_block`'s type and publication status but not its password, while WordPress's own `render_block_core_block()` refuses to render a protected reference at all. The gap reached three places at once: the `.md` body, the front-matter `description` fallback (which walks the same synced-pattern expansion when there is no SEO description or excerpt), and the enriched `/llms.txt` (which reuses that same fallback). The check is now identical to core's, read directly off `post_password` rather than through `post_password_required()` — the rule is about the content, not whether the current visitor happens to hold the password, exactly as for an ordinary password-protected post. A password of the literal string `"0"` is treated as a password, deliberately stricter than core's `! empty()`, which reads it as unprotected.
* **Fixed: relative-link resolution normalized dot segments across the whole reference, not just the path.** `ContentRenderer::absolutize()` handed the complete relative URL to `resolve_dot_segments()`, so a `..` sequence sitting inside a query string or a fragment was treated as path navigation. `../asset?return=/a/../b` had its query value rewritten to `/b`, and `../asset#section/../other` resolved to a different document entirely — the fragment's own `..` consumed `asset` itself. The query and fragment are now split off before normalization and reattached verbatim afterward, per RFC 3986 §5.2.2–5.2.4, which resolves components separately. Two further bugs in the segment algorithm itself, both silent: a trailing `..` (`..`, resolving a post at `/blog/my-post/`) dropped the trailing slash instead of leaving one (RFC 3986 §5.2.4 step 2C replaces a final `/..` with `/`, it does not delete it), and the guard against popping an empty segment — meant to protect the root — also protected the empty segment produced by a doubled slash, so `/a//../b` came out `/a//b` instead of `/a/b`.
* **Fixed: a `<dl>` grouping its `<dt>`/`<dd>` pairs inside `<div>` children — a shape the HTML standard permits and which hand-written HTML and some themes produce — was removed from the document, terms and definitions alike.** `flatten_definition_lists()` only recognized direct `dt`/`dd` children, so a grouped list produced an empty replacement fragment, and the empty-fragment branch deleted the whole list. It now also collects `dt`/`dd` pairs one level inside a `div` child (not deeper, so a nested definition list keeps its own entries), and — independently of the grouping shape — an unrecognized `<dl>` is no longer deleted merely for producing an empty fragment: it is now removed only when it carries no text at all, so an unsupported shape degrades to an imperfect conversion instead of silently losing content. A list-only test fixture could not have caught the original defect: on its own the whole document comes back empty and the DOM-pass fallback republishes the raw, un-flattened text, which is why the regression tests pair every `<dl>` case with surrounding prose.
* **Fixed: a conditional request answered `304 Not Modified` regardless of its HTTP method.** `MarkdownController::handle_conditional()` and `LlmsTxtController::handle_conditional()` treated a matching `If-None-Match` as a revalidation on any method, so a `POST` carrying `If-None-Match: *` — which `etag_matches()` accepts unconditionally, by design, for `GET`/`HEAD` — received a body-less `304`. RFC 9110 §13.1.2 reserves `304` for `GET`/`HEAD`; on any other method a failed precondition is a `412`, which this endpoint does not implement, so it now serves the full response instead. On the canonical permalink, content negotiation itself is now skipped for a non-`GET`/`HEAD` request, so a `POST` to a page (a comment form, for instance) is left entirely to WordPress rather than being answered with Markdown or a `406`. The dedicated `.md` URL is unaffected beyond the conditional shortcut: it serves nothing else, so it keeps answering every method with the document.

Both public contracts (`docs/output-format.md`) and the user documentation (`documentation/src/content/docs/settings/enabled-content-types.md`) were updated in the same PR: the pattern-expansion gate, the `<dl>` grouping support, and the GET/HEAD-only conditional-request rule are all observable behaviour changes. `docs/filters.md` is untouched — no filter was added, changed or removed. `README.md` and `readme.txt`'s FAQ were considered and left as they were: nothing here changes what a filter does or what the panel offers. `AGENTS.md` and `docs/staging-acceptance.md` did change, both gaining the protected-pattern and request-method acceptance fixtures.
