...
Both public contracts (`docs/output-format.md`) and the user documentation (`documentation/src/content/docs/settings/enabled-content-types.md`) were updated in the same PR: the pattern-expansion gate, the `<dl>` grouping support, and the GET/HEAD-only conditional-request rule are all observable behaviour changes. `docs/filters.md` is untouched — no filter was added, changed or removed. `README.md`, `readme.txt`'s FAQ, `docs/staging-acceptance.md` and the rest of `AGENTS.md` were considered and left as they were: nothing here changes what a filter does, what the panel offers, or the acceptance matrix's scope (though item 7 and item 10 now additionally exercise the fixed dot-segment and `<dl>` behaviour, and a POST-based conditional check is worth adding to item 13's HTTP matrix on the next staging pass).
