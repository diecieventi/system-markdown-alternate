...
- **Shortcodes**: `[sysmda_md_url]` prints the `.md` URL anywhere, and `[sysmda_md_download]` renders a link that saves the Markdown as a file instead of opening it (`[sysmda_md_download text="Save it"]` for a custom label). The download link is a bare anchor with a single `sysmda-md-download` class — no CSS and no JavaScript are added to your site, so styling is entirely up to your theme.
