...
> `CLAUDE.md` è un **symlink** a questo file: Claude Code, Cursor, Copilot & co.
> leggono la stessa fonte di verità, senza duplicati da tenere allineati. Le poche
> note specifiche di **Claude Code (web)** stanno nella sezione dedicata in fondo
> a "Identità, versioning, workflow"; gli altri agenti le ignorano.
...
```
.
├── AGENTS.md                     ← questo file (guida agnostica)
