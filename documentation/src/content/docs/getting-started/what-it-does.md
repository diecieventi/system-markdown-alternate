...
- Tables keep their shape. Markdown requires a table to have a header row, and the block editor's own header section is off by default — so an ordinary table used to have its first row of *data* published as the column headings, quietly turning values into labels. A table with no header of its own now gets an empty one instead, and merged cells (`colspan`/`rowspan`) are filled out so every value stays under the column it belongs to. A table that does have a real header row is unchanged.
